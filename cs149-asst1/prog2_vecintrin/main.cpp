...
void clampedExpVector(float *values, int *exponents, float *output, int N) {
    ...
    __cs149_vec_float value_x;
    __cs149_vec_int exponent_x;
    __cs149_vec_float result;
    __cs149_vec_float clampValue = _cs149_vset_float(9.999999f);

    __cs149_vec_int exp_zero = _cs149_vset_int(0);
    __cs149_vec_int exp_ones = _cs149_vset_int(1);

    __cs149_mask maskElement, maskMultiply, maskClamp;

    for (int i = 0; i < N; i+=VECTOR_WIDTH){
        int kElements = N - i;
        maskElement = _cs149_init_ones(kElements);

        value_x = _cs149_vset_float(1.0);
        exponent_x = _cs149_vset_int(0);

        maskMultiply = _cs149_init_ones();
        maskMultiply = _cs149_mask_not(maskMultiply);

        maskClamp = _cs149_init_ones();
        maskClamp = _cs149_mask_not(maskClamp);

        result = _cs149_vset_float(1.0);

        _cs149_vload_float(value_x, values, maskElement);
        _cs149_vload_int(exponent_x, exponents, maskElement);
        _cs149_vgt_int(maskMultiply, exponent_x, exp_zero, maskElement);

        while(_cs149_cntbits(maskMultiply) > 0){
            _cs149_vmult_float(result, value_x, result, maskMultiply);
            _cs149_vsub_int(exponent_x, exponent_x, exp_ones, maskMultiply);
            _cs149_vgt_int(maskMultiply, exponent_x, exp_zero, maskElement);
        }

        _cs149_vgt_float(maskClamp, result, clampValue, maskElement);
        _cs149_vmove_float(result, clampValue, maskClamp);

        _cs149_vstore_float(output, result, maskElement);
        values += VECTOR_WIDTH; exponents += VECTOR_WIDTH; output+= VECTOR_WIDTH;
    }
